...
plot4<-function(){
      ...
      this.df <- data_from_sector("coal")        ## get data that matches this string
      ...
      g + geom_point(alpha = 1/3) +
            geom_smooth(method="lm", col="steelblue",aes(group = 1)) +
            xlab("Year") + ylab("Total PM2.5 Emissions (1000 tons)") +
            ggtitle("Downward trend in emissions for coal combustion-related sources (all US)")+
